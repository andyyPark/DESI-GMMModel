...
class GMMModel(object):
    ...
    def __init__(self, x1_edges, x2_edges):
        ...
        self.areas = np.diff(x2_edges).reshape(-1, 1) * np.diff(self.x1_edges)
    ...
    def gauss(self, mu1, mu2, sigma1, sigma2, rho, moments=False):
        ...
        result = np.zeros(shape=(6, n2, n1)) if moments else np.zeros(shape=(1, n2, n1)) 
    ...
    def get_Cinv(sigma1, sigma2, rho):
        ...
        C12 = S12 * rho
        detC = S12 ** 2 - C12 ** 2
        print(np.array(((sigma2 ** 2, -C12), (-C12, sigma1 ** 2))) / detC)
